      ...
      {isRouteResolved && isSharedView && (
        <header className="h-12 flex items-center px-4 sm:px-10 justify-between bg-transparent z-10 shrink-0">
          <div className="flex items-center gap-2 text-xs font-bold tracking-tight text-ink-main">
            ...
            <span className="hidden sm:inline tracking-wide">FLIP-D // {`${(currentPage).toString().padStart(2, '0')}-${(totalPages > 0 ? totalPages - 1 : 0).toString().padStart(2, '0')}`}</span>
            <span className="sm:hidden tracking-wide">FLIP-D // {`${(currentPage).toString().padStart(2, '0')}-${(totalPages > 0 ? totalPages - 1 : 0).toString().padStart(2, '0')}`}</span>
          </div>
          <a 
            href="/" 
