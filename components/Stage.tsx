...
export const Stage: React.FC<StageProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
        ...
        const verticalPadding = 120; // Header + Toolbar + Margins
  ...
  if (!pdfFile) {
      return (
        <section className="flex-1 w-full h-full p-6 flex flex-col relative z-10">
            <div 
                className={`flex-1 w-full border-2 border-dashed transition-all duration-300 rounded-lg flex flex-col items-center justify-center gap-10 ${isDragging ? 'border-sys-accent bg-sys-accent/5' : 'border-ink-main'}`}
            ...
            >
                ...
                <div className="relative group">
                    <div className="w-[320px] h-[220px] bg-white border border-gray-100 shadow-xl flex items-center justify-center relative z-10 transition-transform duration-500 group-hover:scale-105">
                        <span className="text-xl font-bold text-gray-300 tracking-widest font-mono">PDF &rarr; BKP</span>
                    </div>
                    {/* Stacked element */}
                ...
                <div className="text-center space-y-3 z-10">
                    <h2 className="text-2xl font-bold tracking-widest text-ink-main">DROP PDF TO BEGIN</h2>
                    <p className="text-[10px] font-bold tracking-[0.2em] text-ink-dim uppercase">or click to browse local files</p>
                </div>

                <button 
                    onClick={() => fileInputRef.current?.click()}
                    className="px-8 py-3 border border-ink-main text-xs font-bold tracking-widest hover:bg-ink-main hover:text-white transition-colors uppercase z-10 bg-transparent"
                >
                    Browse Files
                </button>
                <input 
  ...
  return (
    <section className="flex-1 relative flex flex-col items-center justify-center p-4 overflow-hidden">
